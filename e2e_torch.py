...
train_inventory = inventory_level[:train_horizon]  # Training inventory (first 60 days)
test_inventory = inventory_level[train_horizon:horizon]  # Testing inventory (last 30 days)

train_demand = future_demand[:train_horizon]  # Training demand (first 60 days)
...
train_time = np.arange(0, train_horizon)  # Time axis for training data (0 to 59 days)
...
def plot_inventory_level(train_time, train_inventory, test_time, test_inventory):
    plt.figure(figsize=(10, 6))
    plt.plot(train_time, train_inventory, label='Training Inventory Level')
    plt.plot(test_time, test_inventory, label='Testing Inventory Level')
...
def plot_demand_forecast(train_time, train_demand, test_time, test_demand, forecasted_demand):
    plt.figure(figsize=(10, 6))
    plt.plot(train_time, train_demand, label='Actual Demand (Training)')
    plt.plot(test_time, test_demand, label='Actual Demand (Testing)')
...
forecasted_demand_aggregated = forecasted_demand.mean(axis=1)  # Shape: (160,)

# Downsample by averaging
samples_per_day = len(forecasted_demand_aggregated) // test_horizon  # 160 // 30 = 5

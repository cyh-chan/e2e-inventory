...
# # Store DataFrames in a dictionary with composite key (sku, store)
# data_frames = {}
...
# # Store the DataFrame in the dictionary with (sku, store) as the key
# data_frames[(sku, store)] = data

# # Save each DataFrame separately
# output_dir = 'inventory_data'
# os.makedirs(output_dir, exist_ok=True)
#
# for (sku, store), df in data_frames.items():
#     # Save as parquet
#     filename = f'inventory_sku_{sku}_store_{store}'
#     df.to_parquet(f'{output_dir}/{filename}.parquet')
#     # Optional: Save as CSV
#     df.to_csv(f'{output_dir}/{filename}.csv', index=False)
#
# print(f"Files saved in directory: {output_dir}")
# print(f"Total number of DataFrames: {len(data_frames)}")
#
# # Verify the size of a sample DataFrame
# sample_key = list(data_frames.keys())[0]
# sample_df = data_frames[sample_key]
# print(f"\nSample DataFrame (SKU={sample_key[0]}, Store={sample_key[1]}) shape: {sample_df.shape}")
